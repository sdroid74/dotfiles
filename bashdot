    ...
    for file in $(ls -a ~); do
        ...
        if [[ -h ~/$file ]]; then
            ...
            for bashdot_dir in $(cat $bashdot_config_file); do
                ...
                readlink ~/$file |egrep "^$bashdot_dir/[a-zA-Z0-9_-]*/$expected_target_file_name$" > /dev/null
                if [ $? -eq 0 ];then
                    echo $file
                fi
            done
...
uninstall() {
    ...
    for link in $(list_links);do
        ...
        expected_target_file_name=`basename $link | cut -c 2-`
        echo $target |egrep "^$dir/$profile/${expected_target_file_name}$" > /dev/null
        if [ $? -eq 0 ]; then
    ...
    for link in $(list_links); do
        ...
        expected_target_file_name=`basename $link | cut -c 2-`
        echo `readlink ~/$link` |egrep "^$dir/[a-zA-Z0-9_-]*/${expected_target_file_name}$" > /dev/null
        if [ $? -eq 0 ]; then
